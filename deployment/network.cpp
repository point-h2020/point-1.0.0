...
void Domain::printDomainData() {
    ...
    for (size_t i = 0; i < network_nodes.size(); i++) {
        ...
        cout << "*****bridge (when operating_system=ovs) " << nn->bridge << endl;
	cout << "*****sdn_implementation (if SDN)" << nn->sdn_implementation <<endl;
			...
			if (nn->connections[j]->LID.zero()) {
				...
				for (unsigned int k = 0; k < an_connections; k++) {
					...
					if (ac->dst_label == nn->label) {
						cout << "bidirectional LID for " << nn->label << "<->" << an->label << endl;
						ac->LID = LID;
					}
				}
...
//    string curr_bridge = "";
    ...
    for(size_t i=0; i < network_nodes.size(); i++){
        ...
        if(nn->operating_system.compare("ovs") == 0){
            //!!! Notice that the following assumes each port to have a single connections (i.e. p2p), therefore number of connections = number of ports. however this may not be the case when there is duplicate though in ovs this is not the norm
		...
		if(nn->sdn_implementation.compare("bridges") == 0) {
		  ...
		  flow_conf << "ovs-ofctl -O OpenFlow13 add-flow s" << nc->src_pt
			    << " \"dl_dst=00:00:00:00:00:00,"
			    <<" table=0,ipv6,priority=100, action= drop\"" << endl;
		  flow_conf << "ovs-ofctl -O OpenFlow13 add-flow s" << nc->src_pt
			    << " \"dl_dst=00:00:00:00:00:00,ipv6_src="
			    << ipv6_h1 << "/" << ipv6_h1
			    << ",ipv6_dst=" <<ipv6_h2 << "/" << ipv6_h2
			    << ",table=0,ipv6,priority=200,action=output:"
			    <<nc->src_pt<<"\"" << endl;
		} else if(nn->sdn_implementation.compare("tables") == 0) {
		  //pipeline implementation
		  if(j < node_no_connections - 1){
                    flow_conf << "ovs-ofctl -O OpenFlow13 add-flow "
			      << nn->bridge << " \"dl_dst=00:00:00:00:00:00, table="
			      << j << ", ipv6,priority=100, action="
			      << "resubmit(," << j+1 << ")\"" <<endl;
                    flow_conf << "ovs-ofctl -O OpenFlow13 add-flow "
			      << nn->bridge << " \"dl_dst=00:00:00:00:00:00,ipv6_src="
			      << ipv6_h1 << "/" << ipv6_h1
			      << ",ipv6_dst=" <<ipv6_h2 << "/" << ipv6_h2
			      << ",table=" << j
			      << ",ipv6,priority=200,action=output:"
			      <<nc->src_pt<<",resubmit(," <<j+1<<")\"" << endl;
		  } else {
                    flow_conf << "ovs-ofctl -O OpenFlow13 add-flow "
			      << nn->bridge << " \"dl_dst=00:00:00:00:00:00, ipv6_src="
			      << ipv6_h1 << "/" << ipv6_h1 << ",ipv6_dst=" <<ipv6_h2
			      << "/" << ipv6_h2 << ",table=" << j
			      << ",ipv6,priority=200,action=output:"
			      <<nc->src_pt<<"\"" << endl;
                    flow_conf << "ovs-ofctl -O OpenFlow13 add-flow "
...
void Domain::writeClickFiles(bool montoolstub, bool dump_supp, bool no_lnxcap) {
	...
	map<string,string> macmap;
        ...
        if(nn->operating_system.compare("ovs") != 0){
            ...
            click_conf << "globalconf::GlobalConf(TYPE " << node_type <<", MODE " << overlay_mode << ", NODEID " << nn->label << "," << endl;
                ...
                if (overlay_mode.compare("mac") == 0
                    || (overlay_mode.compare("mac_ml") == 0
                        ...
                        if ((offset = findOffset(unique_ifaces, nc->src_if)) == -1) {
                            ...
                            click_conf << unique_ifaces.size() << "," << nc->src_mac << "," << nc->dst_mac << "," << nc->proto_type << "," << nc->LID.to_string();
                        } else {
                            click_conf << offset + 1 << "," << nc->src_mac << "," << nc->dst_mac << "," << nc->proto_type << "," << nc->LID.to_string();
                        }
                ...
                for (size_t j = 0; j < unique_ifaces.size(); j++) {
                    ...
                    if (nn->running_mode.compare("user") == 0) {
                        if (nn->operating_system.compare("Darwin") == 0) {
                            click_conf << "fromdev" << j
                            << "::FromDevice(" << unique_ifaces[j]
                            << ", PROMISC true";
                ...
                for (; it!=ifmap.end(); ++it){
                    ...
                    if (nn->running_mode.compare("user") == 0) {
                        if (nn->operating_system.compare("Darwin") == 0) {
                            ...
                            << macmap[it->first] <<"\""
                            << ");"
                            << endl << "todev_" << it->first
                            << "::ToDevice(" << it->first << ");" << endl;
                        }
                        else {
                            click_conf << "fromdev_" << it->first
                            << "::FromDevice(" << it->first;
                ...
                for (size_t j = 0; j < unique_ifaces.size(); j++) {
                    if (nn->running_mode.compare("kernel") == 0) {
                        ...
                        if(unique_ifaces_end[j].compare("eth_pt") == 0){
                            ...
                            click_conf << "fw[" << (j + 1) << "]->tsf" << j << "->todev" << j << ";" << endl;
                            click_conf << "fromdev" << j << "->classifier" << j << "[0]->[" << (j + 1) << "]fw;" << endl;
                        } else if (montoolstub && j == 0 && (nn->running_mode.compare("user") == 0)) {
                            click_conf << endl << "classifier" << j << "::Classifier(12/080a);" << endl;
                            click_conf << "fw[" << (j + 1) << "]->tsf" << j << "->outc::Counter()->todev" << j << ";" << endl;
                            click_conf << "fromdev" << j << "->classifier" << j << "[0]->inc::Counter()  -> [" << (j + 1) << "]fw;" << endl;
                        } else {
    ...
    for (size_t i = 0; i < network_nodes.size(); i++) {
        ...
        for (size_t j = 0; j < nn->connections.size(); j++) {
            ...
            if ((offset = findOffset(unique_ifaces, nc->src_if)) == -1) {
                unique_ifaces.push_back(nc->src_if);
                if (j == nn->connections.size() - 1) {
                    click_conf << unique_ifaces.size() << "," << nc->src_mac << "," << nc->dst_mac << "," << nc->proto_type << "," << nc->LID.to_string() << ");" << endl << endl;
                } else {
                    click_conf << unique_ifaces.size() << "," << nc->src_mac << "," << nc->dst_mac << "," << nc->proto_type << "," << nc->LID.to_string() << "," << endl;
                }
            } else {
                if (j == nn->connections.size() - 1) {
                    click_conf << offset + 1 << "," << nc->src_mac << "," << nc->dst_mac << "," << nc->proto_type << "," << nc->LID.to_string() << ");" << endl << endl;
                } else {
                    click_conf << offset + 1 << "," << nc->src_mac << "," << nc->dst_mac << "," << nc->proto_type << "," << nc->LID.to_string() << "," << endl;
                }
            }
        }
        for (size_t j = 0; j < unique_ifaces.size(); j++) {
...
void Domain::scpClickFiles() {
    ...
    for (size_t i = 0; i < network_nodes.size(); i++) {
        ...
        if(nn->operating_system.compare("ovs") == 0){
            of_command = "scp " + write_conf + nn->label + ".sh" + " " + user + "@" + nn->testbed_ip + ":" + write_conf;
...
void Domain::startClick(bool log) {
    FILE *ssh_command;
    string command;
	string filename;
	string tm_filename;
    for (size_t i = 0; i < network_nodes.size(); i++) {
        ...
        if(nn->operating_system.compare("ovs") != 0){
            ...
            if (nn->running_mode.compare("user") == 0) {
                if (sudo) {
                    command = "ssh " + user + "@" + nn->testbed_ip + " \"sudo " + click_home + "bin/click " + write_conf + nn->label + ".conf > " + filename + " 2>&1 &\"";
                } else {
                    command = "ssh " + user + "@" + nn->testbed_ip + " \"" + click_home + "bin/click " + write_conf + nn->label + ".conf > " + filename + " 2>&1 &\"";
                }
                cout << command << endl;
                ssh_command = popen(command.c_str(), "r");
                if (ssh_command == NULL) {
                ...
                if (sudo) {
                    command = "ssh " + user + "@" + nn->testbed_ip + " \"sudo " + click_home + "sbin/click-install " + write_conf + nn->label + ".conf > " + filename + " 2>&1 &\"";
                } else {
                    command = "ssh " + user + "@" + nn->testbed_ip + " \"" + click_home + "sbin/click-install " + write_conf + nn->label + ".conf > " + filename + " 2>&1 &\"";
                }
                cout << command << endl;
                ssh_command = popen(command.c_str(), "r");
                if (ssh_command == NULL) {
            ...
            if (sudo) {
                command = "ssh " + user + "@" + nn->testbed_ip + " -t \"sudo chmod +x " + write_conf + nn->label + ".sh; sudo " + write_conf + nn->label + ".sh\"";
            } else {
                command = "ssh " + user + "@" + nn->testbed_ip + " -t \"chmod +x " + write_conf + nn->label + ".sh; " + write_conf + nn->label + ".sh\"";
            }
            cout << command << endl;
            ssh_command = popen(command.c_str(), "r");
            if (ssh_command == NULL) {
...
void Domain::startTM(bool log) {
    FILE *ssh_command;
    string command;
	string filename;
    /*kill the topology manager first*/
    if (sudo) {
    	command = "ssh " + user + "@" + TM_node->testbed_ip + " -t \"sudo pkill -9 tm\"";
    } else {
    ...
    /*now start the TM*/
    if (sudo) {
    	command = "ssh " + user + "@" + TM_node->testbed_ip + " \"sudo /home/" + user + "/blackadder/TopologyManager/tm " + write_conf + "topology.graphml > " + filename + " 2>&1 &\"";
    } else {
    	command = "ssh " + user + "@" + TM_node->testbed_ip + " \"/home/" + user + "/blackadder/TopologyManager/tm " + write_conf + "topology.graphml > " + filename + " 2>&1 &\"";
    }
    cout << command << endl;
    ssh_command = popen(command.c_str(), "r");
    if (ssh_command == NULL) {
...
MergingResult Domain::mergeDomains(Domain &new_dm)
{
	...
	if (new_dm.network_nodes[0]->label.compare("0000000x") == 0)
	{
		...
		for (std::vector<NetworkConnection*>::iterator it = new_dm.network_nodes[0]->connections.begin();
				it != new_dm.network_nodes[0]->connections.end();)
		{
			...
			for (int n=0; n<network_nodes.size(); n++)
			{
				...
				{
					std::cout << "Found attachment node, creating connection." << std::endl;
			...
			if (!attachment_found)
			{
				std::cout << "No attachment node found, removing connection." << std::endl;
		...
		{
			std::cout << "New node has active connections, hence will be added to topology."
					<< std::endl;
	...
	else if (new_dm.network_nodes[0]->label.compare("xxxxxxxx") == 0)
	{
		...
		{
			NetworkNode* nn = *it;
			if (nn->testbed_ip.compare(new_dm.network_nodes[0]->testbed_ip) == 0) {
				std::cout << "Found node to be deleted, removing it." << std::endl;
				...
				it != network_nodes.end();)
		{
			 ...
			 */
			for (std::vector<NetworkConnection *>::iterator it2 = nn->connections.begin();
					it2 != nn->connections.end();)
			{
...
string Domain::generateLabel()
{
	...
	do
	{
		...
		ss << std::setw(ba_id_len) << std::setfill('0') << (++previous_network_size);
		new_label = ss.str();
