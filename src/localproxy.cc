...
void LocalProxy::push(int in_port, Packet * p) {
	...
	bool forward;
	unsigned char type, APItype, numberOfIDs, IDLength /*in fragments of PURSUIT_ID_LEN each*/, prefixIDLength /*in fragments of PURSUIT_ID_LEN each*/, strategy, numberofNodeIDs;
	Vector<String> IDs;
	...
	if (in_port == 2) {
		...
		if (type == PUBLISH_DATA) {
			...
			forward = storeActiveNode(nodeID);
		...
		switch (type) {
				...
				case PUBLISH_DATA:
			{
				...
				switch (strategy) {
					...
					{
						if ((ID.compare(gc->AppnotificationIID) == 0)) {
							/*special case: an app published notification to the core node*/
							p->pull(sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (strategy));
							handleAppNotification(ID, p, _localhost);
							p->kill();
						} else{
							/*Careful: I will not kill the packet - I will reuse it one way or another, so....get rid of everything except the data*/
							p->pull(sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (strategy));
...
bool LocalProxy::storeActivePublication(LocalHost *_publisher, String &fullID, unsigned char strategy, BABitvector &RVFID, bool isScope) {
	...
	if ((strategy == NODE_LOCAL) || (strategy == DOMAIN_LOCAL)) {
		...
		if (ap == activePublicationIndex.default_value()) {
			...
			if (ap->publishers.get(_publisher) == RE_PUBLISH) {
				return true;
			}
...
bool LocalProxy::storeActiveNode(String &_isubscriberID) {
	ActiveNode *an;
			...
			case UPDATE_FID_iSUB:
		{
			...
			for (int i = 0; i < (int) numberOfiSubscribers; i++) {
				...
				if (an != activeNodeIndex.default_value()) {
					...
					click_chatter("LocalProxy: FID added to ActiveNode : %s", isubscriber.quoted_hex().c_str());
				}
			}
			break;
		}
...
/*The method handles app publications to blackadder - notice this is different from handling localsubscribers, which are apps.
 *currently only used by the LinkStateMonitor to notify blackaddder of disconnect/reconnect status (i.e. isolated from the network, connected back)
 */
void LocalProxy::handleAppNotification(String &ID, Packet *p, LocalHost *_localhost) {
	unsigned char type;
	unsigned int numberOfActivePublications = activePublicationIndex.size();
	unsigned int numberOfActiveNodes = activeNodeIndex.size();
	StringSet NodeIDs;
	...
	switch (type) {
			case DISCONNECTED:
		{
			...
			/*failure caused the node to disconnect from the network*/
			/*first notify the disconnection reporting App (i.e. LinkStateMonitor) to stop publishing notifications*/
			sendNotificationLocally(PAUSE_PUBLISH, _localhost, ID);
			for (unsigned int i = 0; i < numberOfActivePublications; i++) {
				if (!(*ap_it).second->isScope) {
					(*ap_it).second->FID_to_subscribers = BABitvector();
					for (PublisherHashMapIter publisher_it = (*ap_it).second->publishers.begin(); publisher_it != (*ap_it).second->publishers.end(); publisher_it++) {
				...
				if (!(*ap_it).second->isScope) {
					for (PublisherHashMapIter publisher_it = (*ap_it).second->publishers.begin(); publisher_it != (*ap_it).second->publishers.end(); publisher_it++) {
						if ((*publisher_it).second == PAUSE_PUBLISH) {
							...
							click_chatter("LocalProxy: resume operation with ID: %s", ap_ID.quoted_hex().c_str());
		...
		default:
			click_chatter("Unkown type %c", type);
			break;
	}
}
...
void LocalProxy::handleUserMulticastRequest(String &ID, unsigned char &type, Packet *p /*the packet has some headroom and only the data which hasn't been copied yet*/, StringSet &isubscriberIDs) {
	...
	if (ap != activePublicationIndex.default_value()) {
		...
		/*no need to clone..packet will be sent only to the network*/
		if (useFatherFID == true) {
			click_chatter("LocalProxy: Multicast FID: %s for scope ID", FID.to_string().c_str());
